...
maestro.get("/directivos-ver-maestros-materias-directores", async (req, res) => {
    ...
    try {
      const result = await client.query(
        `SELECT maestros.id_maestro, numero_documento, nombres, apellidos, codigo_materia, codigo_grupo
        FROM maestros
...
maestro.post('/directivos-nuevo-maestro-persona', async(req,res)=>{
  ...
  try {
      ...
      if (result.rows) {
       ...
       const result2 = await client.query(`INSERT INTO maestros VALUES (NEXTVAL ('maestros_seq'), ${result.rows[0].id_persona}, '${codigo_maestro}');`);
       if(result2){
