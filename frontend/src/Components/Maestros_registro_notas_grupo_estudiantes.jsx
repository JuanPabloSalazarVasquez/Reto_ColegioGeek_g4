...
class Maestros_registro_notas_grupo_estudiantes extends React.Component {
  ...
  componentDidMount() {
    if(this.state.id_grupo == null){
      ...
      axios
      .get(`http://35.185.93.150:4535/grupos-estudiantes/estudiantes-grupo-notas-ver-all-estudiantes/${JSON.parse(sessionStorage.getItem('id_grupo')).id_grupo}`)
      .then((res) => {
      ...
      axios
      .get(`http://35.185.93.150:4535/grupos-estudiantes/estudiantes-grupo-notas-ver-all-estudiantes/${this.state.id_grupo}`)
      .then((res) => {
        console.log(res.data);
        this.setState({
          datos: res.data,
